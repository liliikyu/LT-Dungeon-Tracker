...
const moveHistory=[];
function rememberMove(){moveHistory.push({state:{...state},lastMove:$('lastMove').textContent});}
function undoMove(){
 if(state.busy||!moveHistory.length)return;
 const previous=moveHistory.pop();Object.assign(state,previous.state);
 $('slotInput').value='';$('journeyInput').value='';$('slotError').textContent='';$('journeyError').textContent='';
 $('lastMove').textContent=previous.lastMove;
 $('announcement').textContent='Move undone. '+describe(state.board,state.square)+'.';render();
}
...
function render(){
 ...
 $('undoMove').disabled=state.busy||moveHistory.length===0;$('restart').disabled=state.busy;$('slotInput').disabled=state.busy;$('goButton').disabled=state.busy;
...
async function moveBy(steps){
 ...
 rememberMove();state.busy=true;$('slotError').textContent='';render();
...
function goToSlot(slot){
 ...
 if(offset+Math.min(slot,remaining)!==from)rememberMove();
...
function goToJourneySlot(slot){
 ...
 if(state.busy)return {error:'A move is already in progress.'};
 const before=position();if(before!==slot)rememberMove();setPosition(slot);if(before!==slot)state.turn++;
