    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
        ...
        <div class="dungeon-achievement-list">${achievements.map((achievement) => {
          ...
          return `<label class="dungeon-achievement-row${checked ? " completed" : ""}" data-achievement-key="${esc(key)}">
            ...
            <span class="dungeon-achievement-copy">
              <span class="dungeon-achievement-name">${esc(achievement.name)}</span>
      ...
      const headerHtml = isExpandable ? `<button class="dungeon-toggle" type="button" aria-expanded="${isExpanded ? "true" : "false"}" title="${isExpanded ? "Return to summary" : "Show full list"} ${esc(dungeon.name)}">
        <div class="dungeon-title-wrap">
          ...
          <div class="dungeon-card-submeta">${illustrations.length} ${illustrations.length === 1 ? "illustration" : "illustrations"} · ${trackableItems.length} codex · ${achievements.length} ${achievements.length === 1 ? "achievement" : "achievements"} · ${titleCount} ${titleCount === 1 ? "title" : "titles"}</div>
        </div>
      ...
      const cardContentHtml = `
        ...
        ${monsterSummaryHtml}
        ${summaryHtml}
        ${isExpandable ? `<div class="dungeon-body">${monsterSectionHtml}${uniqueLootSectionHtml}${achievementSectionHtml}${columnsHtml}${titleNote}<section class="dungeon-conquest-placeholder"><div class="dungeon-conquest-placeholder-head"><strong>Dungeon Conquest</strong><span class="coming-soon-badge">Coming Soon</span></div></section></div>` : ""}
      `;
