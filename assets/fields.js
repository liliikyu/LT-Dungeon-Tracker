(() => {
  ...
  const FIELD_ACH = window.LT_FIELD_ACHIEVEMENTS || { fields:{} };
  ...
  function regionFor(name){
    ...
    const explicitAliases={
      ...
      "storm watcher ruins":"Freios",
      "storm weather ruins":"Freios",
      "underworld cave":"Jiendia",
      "snowfield":"Jiendia",
      "soprano snowfield":"Jiendia",
  ...
  const FIELD_ACH_ALIASES={
    "amalrune desert":"amarune desert",
    "pneuma":"pheuma",
    "aie island":"air island",
    "orcarium":"oracrium",
    "invernell ruins":"invernel ruins",
    "lindos sacred place":"linlos sacred place"
  };
  function fieldAchievementsFor(fieldName){
    const fields=FIELD_ACH.fields||{};
    if(Array.isArray(fields[fieldName])) return fields[fieldName];
    const wanted=normField(fieldName);
    const aliasWanted=FIELD_ACH_ALIASES[wanted]||wanted;
    const match=Object.entries(fields).find(([name])=>{
      const n=normField(name);
      return n===wanted || n===aliasWanted || (FIELD_ACH_ALIASES[n]||n)===wanted;
    });
  ...
  const allRegionBox=regionCheckboxes.find(box=>box.value==="all");
  const regionBoxes=regionCheckboxes.filter(box=>box.value!=="all");
  function selectedRegions(){
    if(!allRegionBox || allRegionBox.checked) return new Set(["all"]);
    return new Set(regionBoxes.filter(box=>box.checked).map(box=>box.value));
  }
  function chooseAllRegions(){
  ...
  function renderAchievementSection(field){
    ...
    const rows=list.map(a=>{
      ...
      return `<label class="dungeon-achievement-row${checked?" completed":""}" data-field-achievement-key="${esc(k)}">
        ...
        ${a.points?`<span class="dungeon-achievement-points">${esc(a.points)} pts</span>`:""}
      </label>`;
    }).join("");
    return `<section class="dungeon-achievement-section field-achievement-section">
      <div class="dungeon-achievement-head">
        <div class="dungeon-achievement-title"><span class="dungeon-achievement-heading">Achievement</span><span class="achievement-count">${list.length}</span></div>
        <span class="dungeon-achievement-progress">Done ${done}/${list.length}</span>
      </div>
      <div class="dungeon-achievement-list">${rows}</div>
    </section>`;
  }
  function render(){
    const q=$("field-search").value.toLowerCase().trim();
    const regions=selectedRegions();
    const completionStatus=$("field-completion-status")?.value||"all";
    const visible=entries.filter(f=>{
  ...
  regionBoxes.forEach(box=>box.addEventListener("change",()=>{
    if(box.checked&&allRegionBox) allRegionBox.checked=false;
    if(!regionBoxes.some(item=>item.checked)&&allRegionBox) allRegionBox.checked=true;
    render();
  }));
  $("field-search").addEventListener("input",render);
  $("field-completion-status")?.addEventListener("change",render);
