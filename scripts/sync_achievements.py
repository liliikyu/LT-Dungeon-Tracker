#!/usr/bin/env python3
"""Sync dungeon item-upgrade achievements from Official La Tale Wiki."""
...
def match_dungeons(notes,names):
    nnotes=norm(notes); out=[]
    for name in names:
        if norm(name) and norm(name) in nnotes: out.append(name)
    # Wiki aliases that differ from tracker spelling
    for wiki,tracker in ALIASES.items():
        if norm(wiki) in nnotes and tracker in names and tracker not in out: out.append(tracker)
    return out
...
def parse(html):
    ...
    for table in p.tables:
        ...
        for row in table[hi+1:]:
            ...
            if not name or 'upgrade' not in objective.lower():continue
            dungeons=match_dungeons(notes,names)
            if not dungeons:continue
            points=None
            if pi is not None and pi<len(row):
                m=re.search(r'\d+',row[pi]); points=int(m.group()) if m else None
            rec={'name':name,'objective':objective,'points':points,'category':'Challenge'}
            for dungeon in dungeons:
                bucket=result.setdefault(dungeon,[])
                if rec not in bucket:bucket.append(rec)
    if not result:raise RuntimeError('No dungeon upgrade achievements parsed; keeping existing snapshot')
    return result
