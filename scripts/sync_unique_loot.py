...
# Authoritative corrections for entries that must remain present even if the
# wiki table markup/parser temporarily omits or mangles the Unique Loot cell.
KNOWN_DUNGEON_LOOT = {
    "Twilight Cathedral": [
...
def self_test() -> None:
    ...
    assert KNOWN_DUNGEON_LOOT["Twilight Cathedral"] == [
        "Twilight Accessories", "Modred Textbook", "Ddraig Fflam Watch"
    ]
...
def main() -> None:
    ...
    for dungeon, items in KNOWN_DUNGEON_LOOT.items():
        # These are exact authoritative values, not supplemental additions.
        dungeons[ALIASES.get(dungeon, dungeon)] = list(items)
